(() => {
    ...
    const i18n = window.tecverseI18n || {};
    const t = (key, fallback) => i18n[key] || fallback;

    const getMountedPathPrefix = () => {
        const segments = window.location.pathname.split("/").filter(Boolean);
        const pageIndex = segments.indexOf("exhibitor-registration");
        return pageIndex > 0 ? `/${segments.slice(0, pageIndex).join("/")}` : "";
    };

    const normalizeEndpointUrl = (rawUrl, fallbackPath) => {
        try {
            const url = new URL(rawUrl || fallbackPath, window.location.href);
            const prefix = getMountedPathPrefix();
            const rootEndpointPaths = new Set(["/register", "/check-email", "/check-phone"]);
            if (prefix && url.origin === window.location.origin && rootEndpointPaths.has(url.pathname)) {
                url.pathname = `${prefix}${url.pathname}`.replace(/\/{2,}/g, "/");
            }
            return url.toString();
        } catch {
            return rawUrl || fallbackPath;
        }
    };

    const submitUrl = normalizeEndpointUrl(form?.getAttribute("action") || form?.action, "/register");
    const checkEmailUrl = normalizeEndpointUrl(form?.dataset.checkEmailUrl, "/check-email");
    const checkPhoneUrl = normalizeEndpointUrl(form?.dataset.checkPhoneUrl, "/check-phone");
    ...
    const setSubmitting = (isSubmitting) => {
        ...
        submitButton.innerHTML = isSubmitting
            ? `<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> ${t("submitting", "Submitting...")}`
    ...
    const validateField = (field) => {
        ...
        if (field.required && !value) {
            setError(field, t("fieldRequired", "This field is required."));
            return false;
        }
        ...
        if (validator === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
            setError(field, t("emailInvalid", "Please enter a valid email address."));
            return false;
        }

        if (validator === "indian-mobile" && !/^(?:\+91[-\s]?)?[6-9]\d{9}$/.test(value)) {
            setError(field, t("mobileInvalid", "Please enter a valid Indian mobile number."));
            return false;
        }

        if (validator === "optional-url" && !isValidUrl(value)) {
            setError(field, t("urlInvalid", "Please enter a valid URL starting with http:// or https://."));
            return false;
        }

        if (validator === "optional-gst" && !GST_PATTERN.test(value.toUpperCase())) {
            setError(field, t("gstInvalid", "Please enter a valid 15-character GSTIN."));
            return false;
        }

        if (field.id === "linkedin" && !LINKEDIN_PATTERN.test(value)) {
            setError(field, t("linkedinInvalid", "Please enter a valid LinkedIn profile URL."));
            return false;
        }
    ...
    const checkDuplicate = async (field, endpoint, paramName) => {
        ...
        try {
            ...
            if (body?.data === true) {
                setError(field, body.message || t("valueRegistered", "This value is already registered."));
            }
        } catch {
    ...
    emailField?.addEventListener("blur", debounce(() => checkDuplicate(emailField, checkEmailUrl, "email"), 200));
    phoneField?.addEventListener("blur", debounce(() => checkDuplicate(phoneField, checkPhoneUrl, "phone"), 200));
    ...
    form?.addEventListener("submit", async (event) => {
        ...
        for (let i = 0; i <= lastStep; i += 1) {
            if (!validateStep(i)) {
                alert(t("checkHighlighted", "Form not submitted. Please check the highlighted fields."));
                showStep(i);
                focusFirstInvalid(i);
                return;
            }
        ...
        const optionalFields = new Set(["website", "gst", "linkedin", "specialRequirements", "remarks"]);
        const payload = Object.fromEntries(new FormData(form).entries());
        Object.entries(payload).forEach(([key, value]) => {
            if (typeof value !== "string") return;
            const trimmedValue = value.trim();
            payload[key] = optionalFields.has(key) && !trimmedValue ? null : trimmedValue;
        });
        ...
        try {
            ...
            if (response.ok && result?.success) {
                ...
                const message = result?.message || t("registrationExists", "This registration already exists.");
                alert(`${t("formNotSubmitted", "Form not submitted.")} ${message}`);
                ...
                const message = result?.message || t("submitError", "Something went wrong while submitting your application. Please try again.");
                alert(`${t("formNotSubmitted", "Form not submitted.")} ${message}`);
                showFormError(message);
            }
        } catch {
            const message = t("serverUnreachable", "Unable to reach the server. Please check your connection and try again.");
            alert(`${t("formNotSubmitted", "Form not submitted.")} ${message}`);
            showFormError(message);
        } finally {
            isSubmitting = false;
    ...
    form?.addEventListener("invalid", () => {
        ...
        alert(t("requiredFields", "Form not submitted. Please check the required fields."));
        setTimeout(() => {
